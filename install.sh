...
crave_arch='amd64'
crave_postfix='.bin'
crave_default_location='/data/data/com.termux/files/usr/bin'
...
if [ -p /dev/stdin ] ; then
    ...
    echo -n "Install to system path (default: /data/data/com.termux/files/usr/bin) [y]/n ? "
    read ans
    case $ans in
...
if [[ $install_crave == true ]]; then
    echo -n "Location to install crave [/data/data/com.termux/files/usr/bin]: "
    read crave_location
